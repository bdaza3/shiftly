  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="ml-4 flex items-center gap-4 relative">
              {joinCode ? (
                ...
                <div className="px-3 py-2 rounded bg-gray-50 border border-gray-200 text-sm text-gray-500">{t('joinCodeUnavailable')}</div>
              )}
                  ...
                  <div className="absolute z-50 right-0 top-full mt-2 w-64">
                    <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-4">
                      <div className="flex items-start justify-between mb-2">
                        <div className="text-sm font-semibold">{t('companyJoinCode')}</div>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        >
                          {codeCopied ? <span className="text-white text-xs">{t('copied')}</span> : <span className="text-white text-xs">{common('copy')}</span>}
                        </button>
                      </div>
                      <p className="text-xs text-gray-500 mt-2">{t('shareJoinCode')}</p>
                    </div>
                  </div>
                </>
              )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {employees.length === 0 ? (
          <div className="col-span-full text-gray-500">{t('noEmployees')}</div>
          ...
          visibleEmployees.map((employee) => (
            <div key={employee.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
              ...
              <div className="space-y-2 mb-4">
                ...
                <p className="text-sm text-gray-500">{t('started', { date: new Date(employee.startDate).toLocaleDateString() })}</p>
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-40 flex items-center justify-center pointer-events-auto">
          ...
          <div className="relative bg-white rounded-lg shadow-lg w-full max-w-md p-6 z-50">
            <h3 className="text-lg font-semibold mb-4">{t('inviteByEmail')}</h3>
            <p className="text-sm text-gray-500 mb-3">{t('inviteDescription')}</p>
            ...
            <input type="text" value={firstNameInput} onChange={(e) => setFirstNameInput(sanitizeString(e.target.value, 80))} placeholder={t('firstNameOptional')} className="w-full border p-2 rounded mb-3" />
            <input type="text" value={lastNameInput} onChange={(e) => setLastNameInput(sanitizeString(e.target.value, 80))} placeholder={t('lastNameOptional')} className="w-full border p-2 rounded mb-3" />
            {message && <p className="text-sm text-red-500 mb-3">{message}</p>}
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setShowModal(false)} className="px-3 py-2 rounded border hover:bg-gray-100 transition-colors hover:cursor-pointer">{common('cancel')}</button>
              <button
                type="button"
                onClick={() => {
