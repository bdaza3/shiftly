    ...
    <div className="space-y-6">
      ...
      {loading ? <div className="rounded-xl border border-gray-200 bg-white p-6 text-sm text-gray-500">{common("loading")}</div> : <>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm"><h3 className="text-lg font-semibold text-gray-900">{t("hoursByDay")}</h3><p className="mt-1 text-sm text-gray-500">{t("hoursByDayDescription")}</p><div className="mt-6 space-y-3">{data.days.map((day) => <div key={day.key}><div className="mb-1 flex justify-between text-sm"><span className="text-gray-600">{day.label}</span><span className="font-medium text-gray-900">{day.hours.toFixed(1)}h</span></div><div className="h-2 rounded-full bg-gray-100"><div className="h-2 rounded-full bg-blue-600" style={{ width: `${(day.hours / data.maxDailyHours) * 100}%` }} /></div></div>)}</div></section>
          <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm"><h3 className="text-lg font-semibold text-gray-900">{t("shiftsByRole")}</h3><p className="mt-1 text-sm text-gray-500">{t("shiftsByRoleDescription")}</p><div className="mt-6 space-y-4">{data.roles.length ? data.roles.map(([role, count]) => <div key={role}><div className="mb-1 flex justify-between text-sm"><span className="text-gray-700">{role}</span><span className="font-medium text-gray-900">{t("shiftCount", { count })}</span></div><div className="h-2 rounded-full bg-gray-100"><div className="h-2 rounded-full bg-violet-500" style={{ width: `${(count / totalRoles) * 100}%` }} /></div></div>) : <Empty text={t("noShiftData")} />}</div></section>
        </div>

        <section className="rounded-xl border border-gray-200 bg-white shadow-sm"><div className="border-b border-gray-200 p-6"><h3 className="text-lg font-semibold text-gray-900">{t("employeeWorkload")}</h3><p className="mt-1 text-sm text-gray-500">{t("employeeWorkloadDescription")}</p></div><div className="overflow-x-auto"><table className="w-full"><thead className="bg-gray-50"><tr><HeaderCell>{t("employee")}</HeaderCell><HeaderCell>{t("assignedShifts")}</HeaderCell><HeaderCell>{t("scheduledHours")}</HeaderCell><HeaderCell>{t("shareOfHours")}</HeaderCell></tr></thead><tbody className="divide-y divide-gray-200">{data.employeeWorkload.length ? data.employeeWorkload.map((employee) => <tr key={employee.id}><td className="px-6 py-4 font-medium text-gray-900">{employee.name}</td><td className="px-6 py-4 text-sm text-gray-700">{employee.shifts}</td><td className="px-6 py-4 text-sm text-gray-700">{employee.hours.toFixed(1)}h</td><td className="px-6 py-4 text-sm text-gray-700">{data.totalHours ? Math.round((employee.hours / data.totalHours) * 100) : 0}%</td></tr>) : <tr><td colSpan={4} className="px-6 py-10"><Empty text={t("noWorkloadData")} /></td></tr>}</tbody></table></div></section>

        {data.unassignedCount > 0 && <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-900"><AlertCircle className="mt-0.5 h-5 w-5 shrink-0" /><p className="text-sm">{t("unassignedAlert", { count: data.unassignedCount })}</p></div>}
      </>}
    </div>
  )
}
...
function Metric({ icon, label, value, detail, tone }: { icon: ReactNode; label: string; value: string; detail: string; tone: "blue" | "green" | "purple" | "amber" }) {
  ...
  return <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between"><div><p className="text-sm text-gray-500">{label}</p><p className="mt-2 text-3xl font-bold text-gray-900">{value}</p></div><div className={`rounded-lg p-3 ${colors[tone]}`}>{icon}</div></div><p className="mt-3 text-xs text-gray-500">{detail}</p></div>
}
